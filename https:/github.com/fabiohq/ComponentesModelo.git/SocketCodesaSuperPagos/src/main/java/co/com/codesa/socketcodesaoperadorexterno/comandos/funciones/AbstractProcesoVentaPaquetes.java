...
/*
import co.com.SuperPagos.comandos.CmndPaquetes;
 ...
 */
public abstract class AbstractProcesoVentaPaquetes implements Action,Serializable{
	...
	public Object execute(Object o) throws Exception {
		...
		logger.info("\n");
		logger.info("==> CODESA INICIA PROCESO VENTA DE PAQUETES PARA EL "+Constantes.NOMBRE_OPERADOR
		...
		configConex = (ConfigConexionTrama) datos.action(DatosTrama.GENERA_TRAMA_PAQUETES);
		
		//TRY CATCH PARA MANEJO DE EXCEPCIONES AL MOMENTO DE EJECUPAR OPERACION DE VENTA CON EL OPERADOR EXTERNO
		try {
			/*CmndPaquetes cmndprincipal = new CmndPaquetes();
			tramaResponse = (String) cmndprincipal.CmndPaquetes(configConex.getV_trama_operador(), configConex.getClaveOperador(), logger, Thread.currentThread().getId());
			*/
            ...
            configConex = (ConfigConexionTrama) datos.action( DatosTrama.TRANSFER_REGISTRA );
            ...
            datos.setObjeto(configConex);
            configConex  = ( ConfigConexionTrama )datos.action( DatosTrama.REGISTRA_INTENTO_FALLIDO);
            
            //Se lanza excepcion
            throw new Exception(String.valueOf(operacion)+",1," + login + "," +  smsError);    
		}finally{
			logger.info("\n==>CODESA FINALIZA PROCESO VENTA DE PAQUETES PARA EL "+Constantes.NOMBRE_OPERADOR+" - Hilo :" + Thread.currentThread().getId()
					+"\n=> TIEMPO EN EL QUE INICIÓ   TODO EL PROCESO DE VENTA DE PAQUETES "+tiempollegada
					+"\n=> TIEMPO EN EL QUE FINALIZÓ TODO EL PROCESO DE VENTA DE PAQUETES "+General.obtenerFechaHoraActual(true)
